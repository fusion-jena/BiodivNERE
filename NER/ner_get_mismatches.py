...
if __name__ == '__main__':
    ...
    for pair in pairs:
        ...
        for s_txt, d_txt, s_tags, d_tags in zip(src_txt, dest_txt, src_tags, dest_tags):
            if s_tags != d_tags:
                mismatch[i] = {'sent': s_txt, 'src': s_tags, 'dest': d_tags}
                i += 1
        ...
        sents = [vals['sent'] for vals in mismatch.values()]
        s_tags = [vals['src'] for vals in mismatch.values()]
        d_tags = [vals['dest'] for vals in mismatch.values()]
